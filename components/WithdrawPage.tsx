...
const WithdrawPage: React.FC<WithdrawPageProps> = ({ onBack, onFreeWithdrawClick, balance, currency, onSuccess, userLevel, onGoToUpgrade, processingMode }) => {
  ...
  const handleWithdraw = () => {
    ...
    if (userLevel < 2) {
      setError('Account Level 2 required to withdraw');
      return;
    }
    ...
    if (userLevel < 4 && Number(formData.amount) < 100000) {
      setError('Minimum withdrawal is ₦100,000 (Level 4+ enjoys unlimited/no minimum withdrawal)');
      return;
    }
      ...
      <div className="space-y-6">
        <div className="glass-card p-4 rounded-2xl border-blue-500/20 flex justify-between items-center">
          ...
          <div className="text-right">
            ...
            <span className={`text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-wider ${userLevel >= 2 ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'}`}>
              Level {userLevel}
            </span>
          </div>
        </div>
          ...
          <div className="p-5 rounded-3xl bg-red-500/10 border border-red-500/20 flex flex-col items-center gap-4 text-center shadow-xl shadow-red-500/5 animate-fadeIn">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-400 leading-relaxed max-w-[280px]">
                To qualify for instant node withdrawals, security compliance requires your wallet to be upgraded to <span className="text-red-400 font-bold">Level 2</span>.
              </p>
            </div>
            <button 
            ...
            >
              ...
              Upgrade to Level 2 Now
            </button>
          </div>
        ) : null}

        <div className={`space-y-4 ${userLevel < 2 ? 'opacity-40 pointer-events-none' : ''}`}>
          {/* Amount */}
          <div className="space-y-1.5">
            <div className="relative">
             ...
             <p className="text-[10px] text-amber-400 font-black uppercase tracking-wider pl-1.5">
              {userLevel >= 4 ? 'No Minimum Withdrawal Limit (Unlimited)' : 'Minimum withdrawal: ₦100,000'}
            </p>
          </div>
